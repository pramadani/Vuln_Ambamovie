...
export class UserController {
    ...
    //         // Key yang digunakan oleh express-rate-limit
    //         const key = `rl:${req.ip}`;

    //         // Ambil percobaan login yang tersisa
    //         const ttl = await redisClient.ttl(key); // Sisa waktu kedaluwarsa
    //         const attempts = await redisClient.get(key); // Ambil jumlah percobaan saat ini
    //         const attemptsNumber = attempts ? parseInt(attempts, 10) : 0;
    //         const remaining = 5 - attemptsNumber;

    //         // Tampilkan pesan berdasarkan kondisi
    //         if (remaining <= 0 && ttl > 0) {
    //             res.status(429).json({
    //                 message: `Terlalu banyak percobaan login. Silakan coba lagi dalam ${ttl} detik.`,
    //             });
    //         } else {
    //             res.status(400).json({
